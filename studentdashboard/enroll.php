...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  if (isset($_POST['course'])) {
    ...
    $checkEnrollmentSql = "SELECT * FROM marks WHERE StudentId = '$studentId' AND CourseId	= '$courseId'";
    $checkEnrollmentResult = $db->query($checkEnrollmentSql);

    if ($checkEnrollmentResult->num_rows > 0) {
      die("You are already enrolled in this course.");
    }

    // Insert a new row in the marks table to enroll the student in the selected course with an initial mark of 0
    $enrollSql = "INSERT INTO marks (StudentId, CourseId	, Mark) VALUES ('$studentId', '$courseId', 0)";
    $enrollResult = $db->query($enrollSql);

    if (!$enrollResult) {
      die("Enrollment failed: " . $db->error);
    }
    ...
    <div class="row">
      <div class="col-md-12 grid-margin">
        <div class="d-flex justify-content-between align-items-center">
          <div>
            <h4 class="font-weight-bold mb-0">Enroll in a Course</h4>
          </div>
        </div>
      </div>
    </div>
            ...
            <form method="post">
              <div class="form-group">
                <p class="card-description">
                   <!-- Display the current academic year and filiere id -->
                  <strong>Current Academic Year: </strong><?php echo $currentAcademicYear; ?><br>
                  <strong>Current Filiere ID: </strong><?php echo $currentFiliereId; ?>
                </p>
                <label for="course">Choose a Course:</label>
                <!-- Populate the select dropdown with available courses based on academic year and filiereid -->
